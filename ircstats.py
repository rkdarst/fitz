...
if len(sys.argv) > 1:
    ...
    f = open('irclogs/oftc/#debconf-team.log')
...
re_msg = re.compile('(?P<time>[^ ]+) <.?(?P<nick>[^ :#]*)(?:#[^ ]+)?> (?P<line>.*)')
nicks = { }
lineno = 0
for rawlineno, line in enumerate(f):
    m = re_msg.match(line)
    #print line, m
    if not m: continue
    lineno += 1
    l = m.groupdict()
    if l['nick'] not in nicks:
        nicks[l['nick']] = NickStats()
    #print nicks[l['nick']]
    nicks[l['nick']] += (lineno, l['line'])
